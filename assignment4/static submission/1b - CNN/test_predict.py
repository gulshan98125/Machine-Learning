...
infile = open('train_cnn_model.pkl','rb')
...
test_data_path = '/home/gulshan98125/test_dataset'
all_folders = sorted(os.listdir(test_data_path))
X_test = np.zeros((len(all_folders),156,160,5))

counter = 0
for folder in all_folders:
	inner_path = test_data_path+'/'+folder
	image_names = sorted(os.listdir(inner_path))
	img_arr = np.array([[cv2.imread(inner_path + '/' + img_name, 0)[32:188]] for img_name in image_names])
